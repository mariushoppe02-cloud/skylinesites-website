          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            ...
            <div className="mt-10 space-y-4">
              ...
              <div className="flex items-center gap-4 p-4 rounded-xl bg-[#18181b] border border-zinc-800">
                ...
                <div><p className="text-xs text-zinc-500 font-medium">{t("phone_label")}</p><span className="text-sm text-white font-semibold">{t("phone_value")}</span></div>
              </div>
            </div>
            <div className="mt-8 p-5 rounded-xl border border-[#C9963B]/20 bg-[#C9963B]/5">
              <div className="flex items-center gap-2 mb-1"><div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /><span className="text-xs text-emerald-400 font-semibold uppercase tracking-wide">{t("response_label")}</span></div>
              <p className="text-sm text-zinc-400">{t("response_text")} <span className="text-white font-semibold">{t("response_hours")}</span>.</p>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.2 }}>
            <form onSubmit={handleSubmit} className="p-8 rounded-3xl bg-[#18181b] border border-zinc-800 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div><label className="block text-xs font-semibold text-zinc-400 mb-2 uppercase tracking-wide">{t("name")} *</label><input required type="text" value={form.name} onChange={(e) => setForm({...form, name:e.target.value})} placeholder={t("placeholder_name")} className="w-full px-4 py-3 rounded-xl bg-[#09090b] border border-zinc-700 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-[#C9963B]/60 focus:ring-1 focus:ring-[#C9963B]/30 transition-all" /></div>
                <div><label className="block text-xs font-semibold text-zinc-400 mb-2 uppercase tracking-wide">{t("email")} *</label><input required type="email" value={form.email} onChange={(e) => setForm({...form, email:e.target.value})} placeholder={t("placeholder_email")} className="w-full px-4 py-3 rounded-xl bg-[#09090b] border border-zinc-700 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-[#C9963B]/60 focus:ring-1 focus:ring-[#C9963B]/30 transition-all" /></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div><label className="block text-xs font-semibold text-zinc-400 mb-2 uppercase tracking-wide">{t("phone")}</label><input type="tel" value={form.phone} onChange={(e) => setForm({...form, phone:e.target.value})} placeholder={t("placeholder_phone")} className="w-full px-4 py-3 rounded-xl bg-[#09090b] border border-zinc-700 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-[#C9963B]/60 focus:ring-1 focus:ring-[#C9963B]/30 transition-all" /></div>
                <div><label className="block text-xs font-semibold text-zinc-400 mb-2 uppercase tracking-wide">{t("service")}</label><select value={form.service} onChange={(e) => setForm({...form, service:e.target.value})} className="w-full px-4 py-3 rounded-xl bg-[#09090b] border border-zinc-700 text-sm text-white focus:outline-none focus:border-[#C9963B]/60 focus:ring-1 focus:ring-[#C9963B]/30 transition-all"><option value="">{t("placeholder_service")}</option>{serviceOptions.map((opt) => <option key={opt} value={opt} className="bg-[#09090b]">{opt}</option>)}</select></div>
              </div>
              <div><label className="block text-xs font-semibold text-zinc-400 mb-2 uppercase tracking-wide">{t("message")} *</label><textarea required rows={5} value={form.message} onChange={(e) => setForm({...form, message:e.target.value})} placeholder={t("placeholder_message")} className="w-full px-4 py-3 rounded-xl bg-[#09090b] border border-zinc-700 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-[#C9963B]/60 focus:ring-1 focus:ring-[#C9963B]/30 transition-all resize-none" /></div>
