...
// motion-fähiges <Link> für animierte SPA-Links (kein Vollreload -> keine Intro)
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link key={link.key} href={link.href} className="text-sm text-zinc-400 hover:text-white transition-colors duration-200">{t(link.key)}</Link>
            ))}
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link href={`/${locale}#kontakt`} className="hidden md:flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold bg-gradient-to-r from-[#C9963B] to-[#E8B84B] text-black hover:opacity-90 transition-all duration-200 hover:shadow-[0_0_20px_rgba(201,150,59,0.4)] font-[var(--font-jakarta)]">{t("cta")}</Link>
        ...
        {menuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="lg:hidden glass-nav border-t border-zinc-800/60">
            <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
              {navLinks.map((link, i) => (
                <MotionLink key={link.key} href={link.href} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} onClick={() => setMenuOpen(false)} className="px-4 py-3 text-zinc-300 hover:text-white hover:bg-white/5 rounded-lg transition-all text-sm font-medium">{t(link.key)}</MotionLink>
              ))}
              <div className="flex items-center gap-3 pt-3 mt-2 border-t border-zinc-800/60">
                ...
                <Link href={`/${locale}#kontakt`} onClick={() => setMenuOpen(false)} className="flex-1 text-center py-2.5 rounded-full text-sm font-semibold bg-gradient-to-r from-[#C9963B] to-[#E8B84B] text-black font-[var(--font-jakarta)]">{t("cta")}</Link>
              </div>
            </div>
          </motion.div>
        )}
